...
func TestTool_ReadFile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"path": "main.go", "offset": 3.0, "limit": 2.0},
			git:     map[string]fakeResult{"show": {out: fileContent}},
			wantSub: "3: line3",
		},
		{
...
func TestTool_ReadBaseFile(t *testing.T) {
	...
	result, isErr := ex.ExecuteTool("read_base_file", map[string]any{"path": "main.go"})
	if isErr {
...
func TestTool_Grep(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: map[string]any{"pattern": "x", "max_results": 2.0},
			git: map[string]fakeResult{
...
func TestTool_GitLog(t *testing.T) {
	...
	result, isErr := ex.ExecuteTool("git_log", map[string]any{})
	if isErr {
		t.Fatalf("unexpected error: %s", result)
	}
...
func TestTool_GitShow(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "file at commit",
			args:    map[string]any{"commit": "abc123", "path": "main.go"},
...
func TestTool_GitStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ex := newTestExecutor()
			ex.gitRunner = fakeGit(tt.git)
			result, _ := ex.ExecuteTool("git_status", map[string]any{})
			if !strings.Contains(result, tt.wantSub) {
				t.Errorf("result should contain %q, got: %s", tt.wantSub, result)
			}
		})
...
func TestTool_GhPRView(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "happy path",
			args:    map[string]any{"pr_number": "42"},
		...
		{
			name:    "gh error",
			args:    map[string]any{"pr_number": "999"},
...
func TestTool_GhPRDiff(t *testing.T) {
	...
	result, _ := ex.ExecuteTool("gh_pr_diff", map[string]any{"pr_number": "42"})
...
func TestTool_GhPRChecks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ex := newTestExecutor()
			ex.cmdRunner = fakeCmd(tt.cmd)
			result, _ := ex.ExecuteTool("gh_pr_checks", map[string]any{"pr_number": "42"})
			if !strings.Contains(result, tt.wantSub) {
				t.Errorf("result should contain %q, got: %s", tt.wantSub, result)
			}
		})
...
func TestTool_GhPRComments(t *testing.T) {
	...
	result, _ := ex.ExecuteTool("gh_pr_comments", map[string]any{"pr_number": "42"})
...
func TestTool_GhPRFiles(t *testing.T) {
	...
	result, _ := ex.ExecuteTool("gh_pr_files", map[string]any{"pr_number": "42"})
...
func TestTool_GhIssueView(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "happy path",
			args:    map[string]any{"issue_number": "10"},
...
func TestTool_GetReview(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, _ := ex.ExecuteTool("get_review", map[string]any{})
			if !strings.Contains(result, tt.wantSub) {
				t.Errorf("result should contain %q, got: %s", tt.wantSub, result)
			}
		})
...
func TestTool_Unknown(t *testing.T) {
	ex := newTestExecutor()
	result, isErr := ex.ExecuteTool("bogus_tool", map[string]any{})
	if !isErr {
...
func TestIntegration_ReadFile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"path": "internal/ai/tools.go"},
			wantSub: "package ai",
		},
		{
			name:    "with offset and limit",
			args:    map[string]any{"path": "go.mod", "offset": 1.0, "limit": 3.0},
			wantSub: "lines 1-3",
		},
		{
			name:    "nonexistent file returns error",
			args:    map[string]any{"path": "this_file_does_not_exist_abc123.go"},
			wantErr: true,
		},
		{
...
func TestIntegration_ReadBaseFile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"path": "go.mod", "offset": 1.0, "limit": 2.0},
			wantSub: "lines 1-2",
		},
		{
			name:    "nonexistent file returns error",
			args:    map[string]any{"path": "no_such_file_xyz.go"},
			wantErr: true,
		},
	}
...
func TestIntegration_ListDir(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"path": "nonexistent_dir_xyz"},
			wantSub: "empty or does not exist",
		},
	}
...
func TestIntegration_Glob(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"pattern": "tools.go"},
			wantSub: "tools.go",
		},
		{
			name:    "doublestar pattern",
		...
		{
			name:    "no matches",
			args:    map[string]any{"pattern": "**/*.xyz_nonexistent"},
			wantSub: "No files match",
		},
	}
...
func TestIntegration_Grep(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:    map[string]any{"pattern": "ToolExecutor"},
			wantSub: "ToolExecutor",
		},
		{
			name:    "with file glob filter",
			args:    map[string]any{"pattern": "ToolExecutor", "file_glob": "*.go"},
			wantSub: "ToolExecutor",
		},
		{
			name:    "fixed string match",
			args:    map[string]any{"pattern": "package ai", "regex": "false"},
			wantSub: "package ai",
		},
		{
			name:    "no matches returns message",
			args:    map[string]any{"pattern": "zzz_n" + "omatch_42"},
			wantSub: "No matches found",
		},
		{
			name:    "max_results capping",
			args:    map[string]any{"pattern": "func", "max_results": 3.0},
			wantSub: "match(es)",
		},
	}
...
func TestIntegration_GitDiff(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			baseRef: "HEAD",
			headRef: "HEAD",
			args:    map[string]any{"paths": "go.mod"},
			wantSub: "No differences",
		},
		{
...
func TestIntegration_GitLog(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "with max",
			args:    map[string]any{"max": 3.0},
			wantSub: "", // just verify no error
		},
		{
			name:      "with rev_range",
			args:      map[string]any{"rev_range": "HEAD~2..HEAD", "max": 5.0},
			wantSub:   "", // just verify no error
...
func TestIntegration_GitShow(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    map[string]any{"commit": "HEAD", "path": "go.mod"},
			wantSub: "module",
		},
		{
...
func TestIntegration_GitBlame(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: map[string]any{"path": "tools.go"},
		},
		{
			name: "blame with line range",
			args: map[string]any{"path": "tools.go", "line_start": 1.0, "line_end": 3.0},
		},
		{
			name:    "missing path",
			args:    map[string]any{},
...
func TestIntegration_GitStatus(t *testing.T) {
	...
	result, isErr := ex.ExecuteTool("git_status", map[string]any{})
	if isErr {
		t.Fatalf("unexpected error: %s", result)
	}
	if result == "" {
...
func TestIntegration_UnknownTool(t *testing.T) {
	ex := newRealExecutor()
	result, isErr := ex.ExecuteTool("fake_tool_xyz", map[string]any{})
	if !isErr {
		t.Error("expected isError=true for unknown tool")
	}
	if !strings.Contains(result, "unknown tool") {
...
func TestIntegration_ReadFileThenReadBaseFile(t *testing.T) {
	...
	headResult, isErr := ex.ExecuteTool("read_file", map[string]any{"path": "go.mod"})
	if isErr {
	...
	baseResult, isErr := ex.ExecuteTool("read_base_file", map[string]any{"path": "go.mod"})
	if isErr {
...
func TestIntegration_AllToolsExecutable(t *testing.T) {
	...
	for _, td := range CanonicalToolDefs() {
		t.Run(td.Name, func(t *testing.T) {
			...
			result, _ := ex.ExecuteTool(td.Name, map[string]any{})
			if result == "" {
