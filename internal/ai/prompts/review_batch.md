...
Your job is COVERAGE — surface every potential issue in the CHANGES, including uncertain ones. Lean toward reporting over silent omission, but every finding must stand on its own: cite file and line, name the concrete impact, and provide verifiable evidence. Use `confidence_score` to triage uncertain calls rather than dropping them.
    ...
    "findings": [
      {
        ...
        "evidence_snippet": "verbatim copy of 1-3 lines from the cited file:line that prove the issue",
...
- `evidence_snippet` is REQUIRED on every finding — a verbatim copy of 1-3 lines from the cited file near `line`. Downstream recheck reads it to locate the suspect code; paraphrases or fabricated snippets get the finding dropped.
...
```json
{
  ...
  "evidence_snippet": "if claims.ExpiresAt <= now {",
